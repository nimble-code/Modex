...
/* Original version by Shaun Flisakowski, Jul 12, 1997 */
...
typedef struct symentry {
	...
	char  used;		/* for struct elements, to see if used */
	char  generated;	/* avoid generating typedefs more than once */
	int   ln;		/*	linenumber */
	char *fn;		/*	file name */
...
symentry_t *mk_enum_const(str_t *sym, treenode *);
symentry_t *mk_label(str_t *sym, treenode *);
symentry_t *mk_tag(str_t *sym, treenode *);
symentry_t *mk_component(str_t *sym, treenode *, treenode*);
...
symentry_t *hashtab_lookup(hashtab_t *, str_t *nme);
symentry_t *hashtab_insert(hashtab_t *, symentry_t *entry);
...
symentry_t *scopetab_lookup(scopetab_t *, str_t *nme);
symentry_t *scopetab_find(scopetab_t *, str_t *nme);
symentry_t *scopetab_insert(scopetab_t *, symentry_t *entry);
...
symentry_t *symtab_lookup(symtab_t *, str_t *nme);
symentry_t *symtab_insert(symtab_t *, symentry_t *);
symentry_t *symtab_insert_at(symtab_t *, symentry_t *, int level);

int         st_enter_scope(symtab_t *);
void        st_exit_scope(symtab_t *);
void        save_fcts(scopetab_t *, FILE *);
...
void        exit_scopes(context_t *, int newlev);
