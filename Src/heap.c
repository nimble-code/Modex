...
Heap *
CreateHeap( uint chunk_size, uint chnk_hunk_ratio )
{	Heap    *heap;
	uint     unit;

	heap = (Heap *) e_malloc(sizeof(Heap));
	...
	heap->hunks = (void **) e_malloc(MIN_HUNK_ARRY * PNTR_SZE);
...
{	int     asze, chnk;
	...
	{	chnk = (heap->next_chunk)++;
		...
		if (chnk == (int) heap->ch_ratio)
		{
			...
			{	asze = heap->hunk_array_sze * 2;	/* double array size */
				pnt = (void *) e_malloc(asze * PNTR_SZE);
			...
			heap->hunks[heap->num_hunks] =
				(void *) e_malloc(heap->chnk_sze * heap->ch_ratio);
	
			if (0) printf("2 chunk allocate at %p -- %d * %d bytes\n",
				heap->hunks[heap->num_hunks],
	...
	{	heap->hunks[0] = (void *) e_malloc(heap->chnk_sze * heap->ch_ratio);
		if (0) printf("1 chunk allocate at %p -- %d * %d bytes\n",
