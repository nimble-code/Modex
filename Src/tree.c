...
copy_tree(treenode *root)
{
    ...
    case LEAF_T:
        leaf = (leafnode *) root;
        newleaf = make_leaf(leaf->hdr.type);
    ...
    case NODE_T:
        newnode = make_node(root->hdr.type);
...
rightmost(treenode *root)
{
    ...
    case NONE_T:
    default:
        fprintf(stderr, "%s: parse error: unknown node %s in rightmost;"
        "  type: %s\n", progname, print_ptr(root), name_of_nodetype(root->hdr.which));
        break;
    }
